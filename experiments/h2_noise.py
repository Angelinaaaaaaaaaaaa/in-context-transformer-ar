...
def run_experiment(p: int,
    ...
    train_sequences, train_weights = generate_ar_dataset(
        n_sequences=n_train, p=p, d=d, T=T, noise_std=0.0,  # Clean
        same_dynamics=False, seed=seed
    )

    print(f"Generating clean validation data...")
    val_sequences, val_weights = generate_ar_dataset(
        n_sequences=n_val, p=p, d=d, T=T, noise_std=0.0,  # Clean
    ...
    test_sequences, test_weights = generate_ar_dataset(
        n_sequences=n_test, p=p, d=d, T=T, noise_std=noise_std,  # Noisy
        same_dynamics=False, seed=seed + 2
    )
